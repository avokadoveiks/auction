...
function initGame() {
	...
	window.addEventListener('error', function (evt) {
		...
		appendChatMessage('Система', `Ошибка: ${evt.message}`, 'system');
	});
	...
	window.addEventListener('unhandledrejection', function (evt) {
		...
		appendChatMessage('Система', `Ошибка (promise): ${reason}`, 'system');
	});
}
...
function makeBet(columnId, accountIndex = currentAccountIndex, isAuto = false) {
	...
	if (isCurrentLeader) {
		if (!isAuto && accountIndex === currentAccountIndex) {
			appendChatMessage("Система", "Ты уже лидируешь в этой колонке. Подожди, пока тебя перебьют.", "system");
		}
		return false;
	}
...
function showWinnerOverlay(columnId, account, amount) {
	...
	overlay.innerHTML = `
		<div class="winner-center">
			...
			<div class="winner-name">${escapeHtml(account?.name ?? 'Игрок')}</div>
			<div class="winner-amount">+${formatNumber(amount)} монет</div>
			<div class="winner-countdown" id="winner-countdown-${columnId}">15</div>
		</div>
	`;
	...
	state.winnerInterval = setInterval(() => {
		state.winnerTimer -= 1;
		const cd = document.getElementById(`winner-countdown-${columnId}`);
		if (cd) cd.textContent = String(state.winnerTimer);
...
function updateTimerDisplay(columnId) {
	...
	if (state && element) {
		...
		// urgent visual when <=5 seconds (including 0) or during zeroGrace
		const urgent = (typeof state.timer === 'number' && state.timer <= 5) || (state.zeroGrace && state.zeroGrace > 0);
...
function updateBonus(columnId) {
	...
	if (state.bank >= 1000) {
		bonusPercent = 3;
	} else if (state.bank >= 500) {
		bonusPercent = 2;
	} else if (state.bank >= 100) {
		bonusPercent = 1;
	}

	element.dataset.bonus = String(bonusPercent);
	if (bonusPercent > 0) {
		element.textContent = `Бонус +${bonusPercent}%`;
...
function toggleAuto(columnId) {
	...
	if (entry.active) {
		...
		appendChatMessage("Система", `${account.name} отключает автоставку в колонке "${config.label}"`, "system");
		return;
	}
