  ...
  if (account.balance < config.bet) {
    if (isAuto) {
      if (disableAuto(columnId, accountIndex)) {
        ...
        appendChatMessage(
          "Система",
          `${name} автставка отключена в колонке "${config.label}": нехватка средств`,
          "system"
        );
      }
...
function showWinnerOverlay(columnId, account, amount) {
  ...
  // trigger multi-coin animation towards balance, then bump the balance
  try {
        const to = document.getElementById('currentBalance');
        if (to) {
            to.dataset.bumpDelta = String(amount);
            flyCoins({
              fromEl: overlay,
              toEl: to,
              particles: Math.min(22, Math.max(10, Math.floor(Math.log10(Math.max(1, amount)) * 8))),
              duration: 900,
              sprite: '🪙'
            }).catch(() => {});
        }
